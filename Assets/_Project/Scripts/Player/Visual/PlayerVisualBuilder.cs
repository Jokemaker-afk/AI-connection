...
public static class PlayerVisualBuilder
{
    ...
    public const string RightHandToolSocketName = "RightHandToolSocket";
    ...
    public static BuildResult EnsurePlayerVisual(GameObject playerRoot)
    {
        ...
        Transform thirdPersonSocket = EnsureThirdPersonHandSocket(directionalRoot, controller);
    ...
    static Transform BuildDirectionalBlockModel(Transform visualRoot, CharacterController controller)
    {
        ...
        EnsureRightHandToolSocket(modelContent);

        return modelRoot;
    }

    static void EnsureRightHandToolSocket(Transform modelContent)
    {
        Transform handSocket = modelContent.Find(RightHandToolSocketName)
            ?? modelContent.Find(ThirdPersonToolSocketName);
        if (handSocket == null)
        {
            var socketGo = new GameObject(RightHandToolSocketName);
    ...
    static Transform EnsureThirdPersonHandSocket(Transform directionalRoot, CharacterController controller)
    {
        ...
        Transform modelContent = directionalRoot.Find("ModelContent");
        ...
        EnsureRightHandToolSocket(modelContent);
        Transform handSocket = modelContent.Find(ThirdPersonToolSocketName)
            ?? modelContent.Find(RightHandToolSocketName);

        Transform playerRoot = controller != null ? controller.transform : directionalRoot;
        Transform legacyRootSocket = FindDeepChild(playerRoot, ThirdPersonToolSocketName);
        if (legacyRootSocket != null && legacyRootSocket != handSocket)
        {
            if (Application.isPlaying)
            {
                Object.Destroy(legacyRootSocket.gameObject);
            }
            else
            {
                Object.DestroyImmediate(legacyRootSocket.gameObject);
            }
        }

        if (handSocket != null)
        {
            handSocket.name = ThirdPersonToolSocketName;
        }
